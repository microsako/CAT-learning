...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #input_label = data['labels'][trainable_index.astype(int)]
        input_question = [data['q_ids'][idx] for idx in integer_indices]
        #input_question = data['q_ids'][trainable_index.astype(int)]
        output_label = [data['labels'][idx] for idx in integer_indices2]
        #output_label = data['labels'][target_index.astype(int)]
        output_question = [data['q_ids'][idx] for idx in integer_indices2]
        #output_question = data['q_ids'][target_index.astype(int)]
